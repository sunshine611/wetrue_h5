...
export default class Backend {
  ...
  static nodeApiGetAddressByNames = async (names) => {
    ...
    return new Promise((resolve) => {
      http.get(`${nodeUrl}/v3/names/${names}`).then((res) => {
        if (res.data.id) {
  ...
  static aeknowApiMyToken = (address, contract) => {
  ...
  static aeknowApiSpendTx = (address, limit, offset) => {
  ...
  static aeknowApiTokenTxs = (address, contract, limit, offset) => {
  ...
  static aeMdwApiMyToken = (address, contract) => {
